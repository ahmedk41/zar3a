...
const validateEmail = (email) => {
  ...
  if (COMMON_TYPOS.includes(domain)) return false;

  // Validate format (TLD >= 2 chars, domain name >= 2 chars)
...
export const chooseRole = async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      status: (role === 'AGRO_EXPERT' || role === 'FARMER' || role === 'SUPPLIER') ? 'pending_approval' : 'active'
    });
  } catch (err) {
    console.error(err);
...
export const completeExpertProfile = async (req, res) => {
  try {
    ...
    const hasCV = cvFilePath || (existingProfile && existingProfile.cvFilePath);
    if (!hasCV) {
      return res.status(400).json({ message: "CV/Credentials file is required to complete Agro-Expert registration" });
    }
...
export const updateProfile = async (req, res) => {
  try {
    const { fullName, username, phone, email, tradeLicense, location, farmSize, soilType, specialty, academicDegree, experienceYears, bio, sensorId } = req.body;
    const user = await User.findByPk(req.user.id, {
      include: [
        { model: FarmerProfile, required: false },
        { model: AgroExpertProfile, required: false },
    ...
    if (user.role === 'FARMER') {
      ...
      if (trimmedSensor) {
        ...
        const existingSensor = await FarmerProfile.findOne({
          where: {
            sensorId: trimmedSensor,
...
// ── ADMIN ONLY: General User Approvals (FARMER, SUPPLIER, AGRO_EXPERT) ─────────
...
export const getPendingUsers = async (req, res) => {
  try {
    ...
    return res.json(pendingUsers.map(safeUser));
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: "Server error" });
  }
