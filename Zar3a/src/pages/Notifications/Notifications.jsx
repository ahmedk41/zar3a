        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1.5 font-medium flex items-center gap-2">
              Zar3a Intelligence Hub • {notifications.length} total
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-2">
          {["all", "unread", "order", "system"].map((f) => (
            ...
            >
              {f === "order" ? "Orders" : f.charAt(0).toUpperCase() + f.slice(1)}
              <span className="text-xs px-2 py-0.5 rounded-md bg-white/50 dark:bg-slate-700/50">
                {filterCounts[f]}
              </span>
            </button>
          ))}
          ...
          <AnimatePresence mode="popLayout">
            {filteredNotifications.length === 0 ? (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">No Notifications</h3>
                <p className="text-slate-500 dark:text-slate-400">Everything is running smoothly in your field.</p>
              </motion.div>
            ) : (
              filteredNotifications.map((notif) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!notif.isRead && (
                      <div className="absolute top-6 left-2 w-2.5 h-2.5 bg-emerald-500 rounded-full"></div>
                    )}
                  </motion.div>
                );
              })
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 text-left">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white dark:bg-slate-800 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden">
              <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                ...
                <button onClick={() => setShowSettings(false)} className="w-10 h-10 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center transition-colors text-slate-500"><LuX size={24}/></button>
              </div>
              <div className="p-8 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-white">Order Notifications</h4>
                    <p className="text-xs text-slate-400 font-medium">Get notified when orders are placed</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-white">Product Notifications</h4>
                    <p className="text-xs text-slate-400 font-medium">Get notified when new products are added</p>
                  </div>
                  <div className="w-12 h-6 bg-emerald-500 rounded-full relative shadow-inner"><div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full"></div></div>
                </div>
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-start gap-3">
                  <p className="text-xs font-bold text-blue-700 dark:text-blue-400 leading-relaxed">Modifying these settings affects how you receive critical alerts for your marketplace activity.</p>
                </div>
              </div>
              <div className="p-8 bg-slate-50 dark:bg-slate-900/50">
                <button onClick={() => setShowSettings(false)} className="w-full py-4 bg-slate-800 dark:bg-emerald-600 text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all">Save Changes</button>
              </div>
            </motion.div>
          </motion.div>
        )}
