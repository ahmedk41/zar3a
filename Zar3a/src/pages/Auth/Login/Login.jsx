        ...
        <div className="bg-surface-card/90 dark:bg-slate-900/90 backdrop-blur-3xl border border-white dark:border-slate-800 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-8 md:p-12 relative overflow-hidden">
          ...
          <header className="text-center mb-10">
            ...
            <h2 className="text-4xl font-black text-text-main dark:text-white tracking-tight">
              {t("login.title")} <span className="text-primary-base">{t("login.titleAccent")}</span>
            </h2>
            <p className="text-text-muted dark:text-text-disabled mt-2 font-bold text-sm">{t("login.subtitle")}</p>
          </header>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-text-disabled uppercase tracking-widest ms-1">{t("login.emailLabel")}</label>
              <div className="relative group">
                <FiMail className={`absolute start-4 top-1/2 -translate-y-1/2 z-10 ${errors.email ? "text-red-400" : "text-text-disabled group-focus-within:text-emerald-500"}`} />
                <input {...register("email")} type="text" placeholder={t("login.emailPlaceholder")}
                  className="w-full ps-12 pe-4 py-4 rounded-2xl border-2 bg-surface-secondary/50 dark:bg-slate-800/50 text-text-main dark:text-white placeholder-slate-400 dark:placeholder-slate-500 border-slate-50 dark:border-slate-700 focus:border-emerald-500 outline-none transition-all font-medium" />
              </div>
              {errors.email && <p className="text-xs text-red-500 font-semibold ms-2">{errors.email.message}</p>}
            </div>

            <div className="space-y-1">
              <label className="text-[10px] font-black text-text-disabled uppercase tracking-widest ms-1">{t("login.passwordLabel")}</label>
              <div className="relative group">
                <FiLock className={`absolute start-4 top-1/2 -translate-y-1/2 z-10 ${errors.password ? "text-red-400" : "text-text-disabled group-focus-within:text-emerald-500"}`} />
                <input {...register("password")} type={showPassword ? "text" : "password"} placeholder="••••••••"
                  className="w-full ps-12 pe-12 py-4 rounded-2xl border-2 bg-surface-secondary/50 dark:bg-slate-800/50 text-text-main dark:text-white placeholder-slate-400 dark:placeholder-slate-500 border-slate-50 dark:border-slate-700 focus:border-emerald-500 outline-none transition-all" />
          ...
          <footer className="mt-8 pt-8 border-t border-slate-50 dark:border-slate-800 text-center">
            <p className="text-text-disabled text-sm font-bold">
              {t("login.newHere")}{" "}
              <Link to="/register" className="text-primary-base font-black hover:underline">{t("login.createAccount")}</Link>
            </p>
          </footer>
        </div>
      </motion.div>
