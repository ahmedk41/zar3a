  ...
  if (!user?.isApproved) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <p className="text-gray-600 mb-4">{t("profile.pendingApp")}</p>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">{t("profile.consultationRequests")}</p>
              <h2 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">
                <FiClipboard className="text-purple-600" /> {t("profile.recentConvs")}
              </h2>
            </div>
            <span className="text-sm text-slate-500 dark:text-slate-400">{conversations.length} {t("profile.contacts")}</span>
          </div>
              ...
              {conversations.map((item) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">{t("notif.filter.unread")}</p>
