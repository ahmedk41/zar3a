    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-white dark:bg-slate-900 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-800 p-6"
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">Consultation Requests</p>
              <h2 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">
                <FiClipboard className="text-purple-600" /> Recent Conversations
              </h2>
            </div>
            <span className="text-sm text-slate-500 dark:text-slate-400">{conversations.length} contacts</span>
          </div>
          ...
          {conversations.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 p-8 text-center text-slate-500 dark:text-slate-400">
              No consultations yet. Once a buyer sends a message, the conversation will appear here.
            </div>
          ) : (
            <div className="space-y-4">
              {conversations.map((item) => (
                <Link
                  ...
                  className="flex items-center justify-between gap-4 rounded-3xl border border-slate-200 dark:border-slate-800 p-4 hover:border-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Unread</p>
                    <p className="mt-1 text-lg font-black text-emerald-600 dark:text-emerald-400">{item.unreadCount || 0}</p>
                  </div>
                </Link>
              ))}
            </div>
