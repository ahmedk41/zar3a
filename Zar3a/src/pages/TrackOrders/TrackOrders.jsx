  ...
  return (
    <div>
      <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('nav.trackOrder')}</p>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('track.feed')}</h1>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="min-h-[260px] p-6">
          {loading ? (
            <div className="text-center py-16 text-slate-500 dark:text-slate-400">{t('track.loading')}</div>
          ) : error ? (
            <div className="rounded-3xl border border-rose-200 bg-rose-50 p-8 text-rose-700">{error}</div>
          ) : !items || items.length === 0 ? (
            <div className="text-center py-16 text-slate-500 dark:text-slate-400">{t('track.noTrackedItems')}</div>
          ) : (
                ...
                <div key={item.id} className="rounded-3xl border border-slate-200 p-5 shadow-sm transition hover:border-emerald-300 dark:border-slate-800 dark:bg-slate-900">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{item.description || t('track.noDesc')}</p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    <div className="rounded-3xl bg-slate-50 p-4 dark:bg-slate-950">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-400">{t('market.price')}</p>
                      <p className="mt-2 text-lg font-bold text-slate-900 dark:text-white">{item.price ? `${item.price} ${t('common.egp')}` : 'N/A'}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-50 p-4 dark:bg-slate-950">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-400">{t('market.buyer')}</p>
                      <p className="mt-2 text-lg font-bold text-slate-900 dark:text-white">{item.User?.fullName || 'Unknown'}</p>
                    </div>
                    <div className="rounded-3xl bg-slate-50 p-4 dark:bg-slate-950">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-400">{t('market.seller')}</p>
                    ...
                    <div className="rounded-3xl bg-slate-50 p-4 dark:bg-slate-950">
                      <p className="text-xs uppercase tracking-[0.2em] text-slate-400">{t('market.category')}</p>
                  ...
                  <div className="mt-6 border-t border-slate-100 dark:border-slate-800 pt-6">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Order Status Tracking</p>
                    {item.status === 'CANCELLED' ? (
                      ...
                      <div className="relative flex justify-between items-center w-full px-2 sm:px-8">
                        ...
                        {['PENDING', 'PROCESSING', 'SHIPPED', 'DELIVERED'].map((step, idx) => {
                          ...
                          return (
                            <div key={step} className="relative z-10 flex flex-col items-center gap-3 bg-white dark:bg-slate-900 px-1 sm:px-2">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center border-4 border-white dark:border-slate-900 transition-all duration-500 shadow-sm ${isCompleted ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-400'} ${isActive ? 'ring-4 ring-emerald-500/30 scale-110' : ''}`}>
                                {isCompleted ? <LuCheck size={14} className="stroke-[3]" /> : <span className="text-[10px] font-bold">{idx + 1}</span>}
                              </div>
                              <span className={`text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-center ${isActive ? 'text-emerald-600 dark:text-emerald-400' : isCompleted ? 'text-slate-700 dark:text-slate-300' : 'text-slate-400'}`}>
